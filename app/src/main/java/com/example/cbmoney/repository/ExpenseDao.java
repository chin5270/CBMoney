...
public interface ExpenseDao {
    ...
    // 查詢某年某月的總花費
    @Query("SELECT SUM(expense) FROM expense_table WHERE year = :year AND month = :month")
    LiveData<Integer> getTotalExpenseForMonth(int year, int month);

    // 查詢某年某月的總花費
    @Query("SELECT SUM(expense) FROM expense_table WHERE year = :year AND month = :month AND day = :day")
    LiveData<Integer> getTotalExpenseForDay(int year, int month,int day);

    // 查詢某年某月，他某個類別的總花費
    @Query("SELECT SUM(expense) FROM expense_table WHERE year = :year AND month = :month AND category = :category")
